...
class MatcherSpec: QuickSpec {
  ...
  override func spec() {
    ...
    describe("regex matcher") {
      ...
      let subject = Matcher.term(matcher: regex, generate: placeholder).rule()
    ...
    describe("type matcher") {
      let subject = Matcher.somethingLike(1234).rule()
    ...
    describe("eachLike matcher") {
      ...
      let subject = Matcher.eachLike(arrayItem, min: 4).rule()
      ...
      it("defaults the minimum required in array to 1") {
        let subject = Matcher.eachLike(arrayItem).rule()
